...
from frt_api.maya import MeshMayaHandler
...
def run_after_edit_assemble(rig):
    editing_expression = cmds.getAttr("MetaHumanDNACalib.Expression_Name")
    rig_expression = rig.rig_definition.get_expression_by_name(editing_expression)
    for mesh_in_expression in rig_expression.meshes_in_expression:
        if mesh_in_expression.mesh.name == "cartilage_lod0_mesh" and mesh_in_expression.type in [2, 3]:
            mmh = MeshMayaHandler()
            mmh.set_to_neutral_mesh(rig, "head_lod0_mesh", True, "sculpt_head_lod0_mesh", [])
            mmh.set_to_neutral_mesh(rig, "cartilage_lod0_mesh", True, "sculpt_cartilage_lod0_mesh", [])

            cmds.select("sculpt_cartilage_lod0_mesh", replace=True)
            cmds.select("sculpt_head_lod0_mesh", add=True)

            mel.eval('doWrapArgList "7" { "1", "0.0", "1.0", "2", "1", "1", "0", "0" }')

            vertex_values = cmds.currentVertexState(
                nodeName="MetaHumanDNACalib", expression=editing_expression, meshName="head_lod0_mesh", phase=1
            )
            vertex_positions = []
            for i in range(0, len(vertex_values), 3):
                vertex_positions.append([vertex_values[i], vertex_values[i + 1], vertex_values[i + 2]])
            mmh.set_to_mesh(True, "sculpt_head_lod0_mesh", [], vertex_positions)
...
def run_before_propagate_changes(rig):
    ...
    for mesh_in_expression in rig_expression.meshes_in_expression:
        if mesh_in_expression.mesh.name == "cartilage_lod0_mesh" and mesh_in_expression.type in [2, 3]:
            cmds.delete("sculpt_cartilage_lod0_mesh", constructionHistory=True)
            cmds.delete("sculpt_head_lod0_meshBase")


def run_after_assemble(head_group, head_rig_group, **kwargs):
    if analog_gui_loaded():
        adjust_aim_control(kwargs["rotation"])
    if gui_loaded():
        connect_expressions()
