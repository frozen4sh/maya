...
from mh_character_assembler.core import consts
...
def get_gui_translation():
    eye_aim_control_name = "CTRL_C_eyesAim"
    right_eye_control_name = "CTRL_R_eye"
    # gui_object_name = "CTRL_faceGUI"
    ...
    if cmds.upAxis(q=True, axis=True) == "z":
        translation = [arm[0], arm[1], arm[2] - (aim - eye) / 2]

    return translation


def adjust_head_gui_position():
    if cmds.objExists("headRig_grp"):
        pivot_local = cmds.xform("CTRL_faceGUI", q=True, ws=False, rp=True)
        cmds.xform("CTRL_faceGUI", piv=(0, 0, 0), ws=False)
        # set rotate pivot to root
        cmds.xform("CTRL_faceGUI", ws=True, rp=[0, 0, 0])
        cmds.xform("CTRL_faceGUI", ro=get_scene_orientation(), relative=True)
        cmds.xform("GRP_convergenceGUI", translation=[0.0, 0.0, 0.0], relative=True)

        resolved_translation = get_gui_translation()
        if resolved_translation:
            cmds.select(clear=True)
            cmds.xform("gui", translation=resolved_translation, relative=True)

        cmds.xform("CTRL_faceGUI", piv=pivot_local, ws=False)


def run_after_assemble(head_group, head_rig_group, params):
    if analog_gui_loaded():
        adjust_aim_control()
        adjust_head_gui_position()
...
def adjust_aim_control():
    ...
    orient = get_scene_orientation()

    cmds.xform("aim_grp", ro=orient, relative=False)
...
def cmds_point_constraint(target, obj):
    try:
        cmds.pointConstraint(target, obj)
    except Exception as ex:
