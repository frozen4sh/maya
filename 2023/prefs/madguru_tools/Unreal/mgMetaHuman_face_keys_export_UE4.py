...
def mgMetaHuman_face_keys_export():
	...
	for num in range(0, len(sequencer_names_list)):
		...
		asset_name = actor.get_actor_label().split('_')[1]
		...
		if face_possessable:
			...
			face_control_channel_list = face_control_rig_track.get_sections()[0].get_channels()
			...
			print('Face Animation Keys output tA: ' + str(keys_file.name))
		else:
